...
html;
	...
	public function getColaboradores($tipo, $identificador, $idPeriodo, $existeFechaDiaFestivo){
		...
		foreach (ProrrateoDao::getColaboradoresProrrateo($tipo,$identificador) as $key => $value) {
			...
			$horasExtraPrevio = $this->horasExtraPrevio($TOTAL, $LimiteHorasExtra, $premioAsistencia, $premioPuntualidad);

			$importeHorasExtra = $this->getImporteHorasExtras($value['sal_diario'], $totalHorasExtraIncentivos, $LimiteHorasExtra, $premioAsistencia, $premioPuntualidad, $horasExtraPrevio);
			$numeroHorasExtra = $this->getNumeroHorasExtra($value['sal_diario'], $importeHorasExtra);
			$despensaEnEfectivo =  $this->getDespensaEnEfectivo($TOTAL, $premioAsistencia, $premioPuntualidad, $importeHorasExtra);
			...
			$htmlGuarda .= <<<html
					<input type="hidden" name="count" value="{$count}">
						<input type="hidden" name="datos_colaborador_{$fila}" value="{$value['clave']}|{$value['nombre']} {$value['apellido_paterno']} {$value['apellido_materno']}| {$premioAsistencia} | {$premioPuntualidad} | {$numeroHorasExtra} | {$despensaEnEfectivo} | {$incentivo} | {$primaDominical} | {$domingoLaborado} | {$totalHorasExtraIncentivos} | {$value['catalogo_colaboradores_id']} | {$idPeriodo} | {$identificador} | $totalHorasExtraIncentivos">
html;
			$html .= <<<html
				<tr>
					...
					<!-- 11 --><td style="background: #fbf6b6; text-align:center; vertical-align:middle;">$ {$premioAsistencia}</td>
					<!-- 12 --><td style="background: #fbf6b6; text-align:center; vertical-align:middle;">$ {$premioPuntualidad}</td>
					...
					<!-- 21 --><td style="background: #fbf6b6; text-align:center; vertical-align:middle;">$ {$nuervosValores} </td>
...
html;
	...
	public function horasExtraPrevio($total, $LimiteHorasExtra, $premioAsistencia, $premioPuntualidad){
		...
		return $val;//$total."-".$premioAsistencia."-".$premioPuntualidad."=".$operacion1 ."<".$LimiteHorasExtra;
	}
