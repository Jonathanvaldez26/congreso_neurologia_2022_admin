...
html;
	...
	public function getPremiodePuntualidadColaborador($SDI, $INCENTIVOS, $TOTALINCENTIVOSHORAS, $PREMIOASISTENCIA){
		/*$TOTALINCENTIVOSHORAS = '375.01';
		$SDI = '179.27';
		$INCENTIVOS = ' 182.00';
		$PREMIOASISTENCIA = '125.49';*/
		$operacion = (($SDI*7)*0.1);
		...
		$operacionNueva = round($operacion, 2);
		/*echo "<br>";
		print_r($operacionNueva);
		echo "<br>";
		print_r($PREMIOASISTENCIA);
		echo "<br>";*/


		$operacion = 0;
		if($operacionNueva == $resultado)
			$operacion = $operacionNueva;
		else
			$operacion = $PREMIOASISTENCIA;

		$resultado1 = number_format($operacion, 2, '.', '');

		if($resultado1 > $PREMIOASISTENCIA){
			$resultado1 = $PREMIOASISTENCIA;
		}
