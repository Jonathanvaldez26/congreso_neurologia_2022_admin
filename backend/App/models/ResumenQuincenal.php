...
class ResumenQuincenal implements Crud{
...
sql;
    ...
    public static function getDatosUsuarioLogeado($user){
        $mysqli = Database::getInstance();
        $query=<<<sql
SELECT a.administrador_id, a.nombre, a.usuario, a.perfil_id, a.tipo, a.catalogo_planta_id, a.status, cp.nombre AS nombre_planta
FROM utilerias_administradores a 
INNER JOIN catalogo_planta cp USING (catalogo_planta_id)
WHERE a.usuario LIKE '$user'
sql;
...
sql;
      ...
      public static function getMaxPeriodo(){
        $mysqli = Database::getInstance();
        $query =<<<sql
        SELECT MAX(prorrateo_periodo_id) AS id from prorrateo_periodo where tipo = 'QUINCENAL'
sql;
        return $mysqli->queryOne($query);
      }

      public static function cancelarPeriodo(){
        $mysqli = Database::getInstance();
        $id = ResumenQuincenal::getMaxPeriodo()['id'];
        $query =<<<sql
