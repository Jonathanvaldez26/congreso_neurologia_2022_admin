...
class Incidencia_bk extends Controller{
...
html;
	...
	public function semanales(){
	    ...
	    if($user['perfil_id'] == 6){
	      ...
	      if($user['perfil_id'] == 6) // Si el usuario es de RH
	        $tituloVista = "Incidencia de Recursos Humanos - Planta " . strtolower($user['nombre_planta']);

	      if($user['perfil_id'] == 4 || $user['perfil_id'] == 5 ) // Si el usuario es de perfil personalizado o admin
	        $tituloVista = "Incidencia del depto. " . strtolower($user['nombre']);

	      if($user['perfil_id'] == 1 ){ // Si el usuario es root
	      	$val = 6;
	        $tituloVista = "Incidencia de TODAS LAS PLANTAS ";
	      }
    }
	...
	public function quincenales(){
		...
		if($user['perfil_id'] == 6){
			...
			if($user['perfil_id'] == 6) // Si el usuario es de RH
				$tituloVista = "Incidencias de Recursos Humanos - Planta " . strtolower($user['nombre_planta']);

			if($user['perfil_id'] == 4 || $user['perfil_id'] == 5 ) // Si el usuario es de perfil personalizado o admin
				$tituloVista = "Incidencias del depto. " . strtolower($user['nombre']);

			if($user['perfil_id'] == 1 ) {// Si el usuario es root
				$tituloVista = "Incidencias de TODAS LAS PLANTAS ";
				$val = 6;
			}
		}
	...
	public function historicosSemanales(){
	    ...
	    View::set('header',$this->_contenedor->header($this->getHeader()));
	    View::set('footer',$this->_contenedor->footer($this->getFooter()));
	    View::render("incidencia_historicas");
  }
	...
	public function propiosSemanalesHistoricos(){
		...
		if($user['perfil_id'] == 6){ // Si el usuario es de RH
			$tituloVista = "Incidencias - Planta " . strtolower($user['nombre_planta']) . " - Depto. " . $user['nombre'];
		...
		View::set('busqueda',"/Incidencia/propiosSemanalesHistoricos/");
		View::set('header',$this->_contenedor->header($this->getHeader()));
		View::set('footer',$this->_contenedor->footer($this->getFooter()));
		View::render("incidencia_historicas");
	}
	...
	public function propiosQuincenalesHistoricos(){
		...
		View::set('busqueda',"/Incidencia/propiosQuincenalesHistoricos/");
		View::set('header',$this->_contenedor->header($this->getHeader()));
		View::set('footer',$this->_contenedor->footer($this->getFooter()));
		View::render("incidencia_historicas");
	}
	...
	public function propiosQuincenales(){
		...
		if($user['perfil_id'] == 6){ // Si el usuario es de RH
			$tituloVista = "Incidencias propias de Recursos Humanos - Planta " . strtolower($user['nombre_planta']) . " - Depto. " . $user['nombre'];
			$val = ($user['catalogo_planta_id'] == 1) ? 1 : 4;
		}

		if($user['perfil_id'] == 1){ // Si el usuario es de RH
			$tituloVista = "Incidencias propias - Planta " . strtolower($user['nombre_planta']) . " - Depto. " . $user['nombre'];
			$estatusRH = ($user['perfil_id'] == 1) ? 2 : 0;
			$val = 4; // TIENE INCENTIVOS PROPIOS
		}
...
html;
    ...
    */
    public function getPeriodosHistoricos($tipo, $periodoObtenido){
    	$periodos = IncidenciaDao::getTipoPeriodo($tipo);
    	$option = "";
...
/*
      $horario_id = IncidenciaDao::getUltimoHorario($datos)['catalogo_horario_id'];
      ...
      if($horario_id == ''){
      	...
      	if(count($horarios) > 1){
      		for($i=0; $i<count($horarios); $i++) {
      			$valueHorario = $horarios[$i];
      			if($valueHorario['catalogo_horario_id'] == $horario_id){
      				if( ($i+1) > count($horarios) ){
      					$horario_id = 0;
      				}else{
      					$horario_id = $horarios[$i+1];
      				}
      			}
      		}
      	}else{
...
html;
	...
	if(count(IncidenciaDao::getFechaIncidenciaById($datos)) > 0) {
              if($estatusPeriodo == 0){
                $tabla .= <<<html
                  <a  href="/AdminIncidencia/deleteIncidecia?id={$colaborador['catalogo_colaboradores_id']}&fecha={$fecha_inicio->format('Y-m-d')}&vista={$vista}" class="btn btn-danger eliminar">
...
html;
		...
		if($vista == "quincenales"){
			$vista = "/Incidencia/quincenales/&action=quincenales";
			$tipoPeriodo = "quincenales";
		}

		if($vista == "historicosSemanales"){
			$vista = "/Incidencia/historicosSemanales/&action=historicosSemanales";
			$tipoPeriodo = "historicosSemanales";
		}

		if($vista == "historicosQuincenales"){
			$vista = "/Incidencia/historicosQuincenales/&action=historicosQuincenales";
...
html;
		...
		if($vista == "historicosSemanales")
			$direccionamiento = "/Incidencia/historicosSemanales/&action=historicosSemanales";

		if($vista == "historicosQuincenales")
			$direccionamiento = "/Incidencia/historicosQuincenales/&action=historicosQuincenales";
