...
class Programa implements Crud{

    public static function getAll(){       
      $mysqli = Database::getInstance();
      $query=<<<sql
      SELECT p.*, prof.nombre as nombre_profesor, c.nombre as nombre_coordinador
      FROM programa p
      INNER JOIN profesores prof ON (p.id_profesor = prof.id_profesor)
      INNER JOIN coordinadores c ON (c.id_coordinador = p.id_coordinador)
sql;
...
sql;
    ...
    public static function insert($data){
      ...
      $query=<<<sql
      INSERT INTO programa(id_profesor,	id_coordinador, clave,hora_inicio, hora_fin, fecha, descripcion, url, duracion, status)
      VALUES(:id_profesor,:id_coordinador, :clave,:hora_inicio, :hora_fin, :fecha, :descripcion, :url, :duracion, 1);
sql;

          $parametros = array(
          ':id_profesor' => $data->_id_profesor,
          ':id_coordinador'=>$data->_id_coordinador,
          ':clave'=>$data->_clave,
          ':hora_inicio'=>$data->_hora_inicio,
          ':hora_fin'=>$data->_hora_fin,
          ':fecha'=>$data->_fecha,
