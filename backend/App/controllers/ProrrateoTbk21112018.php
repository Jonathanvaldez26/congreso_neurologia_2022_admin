...
html;
	...
	public function getPremiodePuntualidadColaborador($SDI, $INCENTIVOS, $TOTALINCENTIVOSHORAS, $PREMIOASISTENCIA){
		...
		if( ($TOTALINCENTIVOSHORAS-$PREMIOASISTENCIA) > (($SDI*7)*0.1) ){
			$resultado = (($SDI*7)*0.1);
			$bandera1 = true;
			$resultado1 = (($SDI*7)*0.1); //echo "true:> {$resultado}";
		}
		...
		if ( ($INCENTIVOS-$PREMIOASISTENCIA) < (($SDI*7)*0.1) ){
			...
			$bandera2 = true; //echo "true:> {$resultado}";
		}
	...
	public function getPremioAsistenciaColaborador($SDI = 433.20, $INCENTIVOS = 300, $TOTALINCENTIVOSHORAS = 735.50){
		$ope = (($SDI*7)*0.1);
...
html;
	...
	public function getSumaHorasExtra_IncentivosPeriodos($horasExtra, $sumaIncentivos, $nuervosValores){
		// CHECAR AQUI 
		//echo "horasExtra:> {$horasExtra} <br> sumaIncentivos :> {$sumaIncentivos} <br> nuervosValores:> {$nuervosValores}";
