...
class ResumenSemanalRH implements Crud{
...
sql;
    ...
    public static function getAllColaboradoresPago($pago,$id_administrador,$usuario, $planta){
      $where;
      if($usuario == "root"){
        $where = "";
      }else{
        $where = ($planta != '')? " AND c.identificador_noi = '$planta'" : '';
      }
...
sql;
    ...
    public static function getDatosUsuarioLogeado($user){
        ...
        $query=<<<sql
        SELECT 
          a.*,p.nombre AS nombre_planta
        FROM utilerias_administradores a 
        JOIN catalogo_planta p
        ON a.catalogo_planta_id = p.catalogo_planta_id
...
sql;
      ...
      public static function cancelarPeriodo(){
        $mysqli = Database::getInstance();
        $id = ResumenSemanalRH::getMaxPeriodo()['id'];
        $query =<<<sql
